...
// loading the db model for the movies
const Movie = require("./../models/Movie.model");

// creating the seed function as an IIFE so it is executed whenever this file is called
(async function() {
  try {
    // since it is "dev conditions", cleaning the DB first to avoid data duplication
    await Movie.deleteMany();
    // then seeding the movies array to the db using the Movie model to ensure document consistency
    const createdMovies = await Movie.create(movies);
    // logging a success message accounting -for the creation
    console.log(`DB operation successul: ${createdMovies.length} movies created`);
    // exiting the seeding process once done
    process.exit();
  }
  catch (e) {
    // displaying the error => from either the .deleteMany() or the .create()
    console.error(e);
    // exiting the seeding process once done
    process.exit();
  }
